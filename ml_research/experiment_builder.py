...
class ExperimentBuilder(nn.Module):
    ...
    def run_train_iter(self, x, y):
        ...
        if type(x) is np.ndarray:
            x, y = torch.Tensor(x).float().to(device=self.device), torch.Tensor(
                y
            ).long().to(
                device=self.device
    ...
    def run_training_epoch(self, current_epoch_losses):
        with tqdm.tqdm(
            total=len(self.train_data), file=sys.stdout
    ...
    def run_validation_epoch(self, current_epoch_losses):
        ...
        with tqdm.tqdm(
            total=len(self.val_data), file=sys.stdout
    ...
    def run_testing_epoch(self, current_epoch_losses):
        ...
        with tqdm.tqdm(
            total=len(self.test_data), file=sys.stdout
    ...
    def run_experiment(self):
        ...
        for i, epoch_idx in enumerate(range(self.starting_epoch, self.num_epochs)):
            ...
            save_statistics(
                ...
                if (self.starting_epoch != 0 or i > 0)
                else False,
