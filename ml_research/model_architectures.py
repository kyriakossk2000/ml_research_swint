...
class ConvolutionalNetwork(nn.Module):
    ...
    def build_module(self):
        ...
        for i in range(self.num_layers):  # for number of layers times
            ...
            out = self.layer_dict["conv_{}".format(i)](
                out
            )  # use layer on inputs to get an output
            out = F.relu(out)  # apply relu
            print(out.shape)
            if (
                self.dim_reduction_type == "strided_convolution"
            ):  # if dim reduction is strided conv, then add a strided conv
                self.layer_dict["dim_reduction_strided_conv_{}".format(i)] = nn.Conv2d(
                    in_channels=out.shape[1],
                    kernel_size=3,
                ...
                out = self.layer_dict["dim_reduction_strided_conv_{}".format(i)](
                    out
                )  # use strided conv to get an output
                out = F.relu(out)  # apply relu to the output
            elif (
                self.dim_reduction_type == "dilated_convolution"
            ):  # if dim reduction is dilated conv, then add a dilated conv, using an arbitrary dilation rate of i + 2 (so it gets smaller as we go, you can choose other dilation rates should you wish to do it.)
                self.layer_dict["dim_reduction_dilated_conv_{}".format(i)] = nn.Conv2d(
                    in_channels=out.shape[1],
                    kernel_size=3,
                    out_channels=out.shape[1],
                    padding=1,
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):  # for number of layers
            ...
            out = self.layer_dict["conv_{}".format(i)](
                out
            )  # pass through conv layer indexed at i
            out = F.relu(out)  # pass conv outputs through ReLU
            if (
                self.dim_reduction_type == "strided_convolution"
            ):  # if strided convolution dim reduction then
                out = self.layer_dict["dim_reduction_strided_conv_{}".format(i)](
                    out
                )  # pass previous outputs through a strided convolution indexed i
...
class ResNet(nn.Module):
    def __init__(
        self, img_channels: int, num_layers: int, block, num_classes: int = 1000
    ) -> None:
