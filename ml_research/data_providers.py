...
class MNISTDataProvider(DataProvider):
    ...
    def __init__(
        ...
        data_path = os.path.join("data", "mnist-{0}.npz".format(which_set))
...
class EMNISTDataProvider(DataProvider):
    ...
    def __init__(
        ...
        # separator for the current platform / OS is used
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join("data", "emnist-{0}.npz".format(which_set))
        assert os.path.isfile(data_path), (
            "Data file does not exist at expected path: " + data_path
        )
        # load data from compressed numpy file
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(
        ...
        Args:
            ...
            rng (RandomState): A seeded random number generator.
        """
        data_path = os.path.join(os.environ["DATASET_DIR"], "HadSSP_daily_qc.txt")
        assert os.path.isfile(data_path), (
            "Data file does not exist at expected path: " + data_path
        )
...
class CCPPDataProvider(DataProvider):
    def __init__(
        ...
        Args:
            ...
            rng (RandomState): A seeded random number generator.
        """
        data_path = os.path.join(os.environ["DATASET_DIR"], "ccpp_data.npz")
        assert os.path.isfile(data_path), (
            "Data file does not exist at expected path: " + data_path
        )
        # check a valid which_set was provided
...
class Products10kDataProvider(DataProvider):
    """Data provider for EMNIST handwritten digit images."""
    ...
    def __init__(
        ...
        self.batch_size_pointer = batch_size
        # construct path to data using os.path.join to ensure the correct path
        # separator for the current platform / OS is used
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            "data", "product-10k-{0}_data.npy".format(self.which_set)
        )
        labels_path = os.path.join(
            "data", "product-10k-{0}_labels.npy".format(self.which_set)
        )

        data = np.load(data_path, allow_pickle=True, mmap_mode="r")
        labels = np.load(labels_path, allow_pickle=True, mmap_mode="r")
        inputs = data
        targets = labels
        ...
        print(inputs.shape)
        print(labels.shape)
        self.total_els = labels.shape[0]
        # if flatten:
        #     inputs = np.reshape(inputs, newshape=(-1, 224 * 224))
        # else:
        #     inputs = np.reshape(inputs, newshape=(-1, 3, 224, 224))
        # inputs = inputs / 255.0

        # pass the loaded data to the parent class __init__
        super(Products10kDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng
        )
    ...
    def next(self):
        ...
        if self.current_batch_index >= self.total_els:
            # no more batches in current iteration through data set so start
            # new epoch ready for another pass and indicate iteration is at end
            self.current_batch_index = 0
            self.new_epoch()
        ...
        data = np.load(data_path, allow_pickle=True, mmap_mode="r")
        labels = np.load(labels_path, allow_pickle=True, mmap_mode="r")
        inputs_batch = torch.from_numpy(
            np.copy(
                data[
                    self.current_batch_index : self.current_batch_index
                    + self.batch_size_pointer,
                    :,
                    :,
                    :,
                ]
        ...
        self.current_batch_index += self.batch_size_pointer
